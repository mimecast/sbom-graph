...
| E9  | Unbounded fan-out during enrichment                     | D      | Redis broker                    | Low        | Medium | Low    | Batched dispatch (500), worker_prefetch_multiplier=1, task_acks_late=True, PLUS (added after an actual incident where this fan-out filled Redis to maxmemory and OOM-looped the worker): an `enrichment_queued_at` dispatch-time guard on `enrich_all_packages` that excludes purls already queued-but-not-yet-processed (the freshness filter alone only excludes *completed* work, so a stalled worker pool caused every beat tick to re-dispatch a duplicate batch on an already-stuck backlog), and a queue-depth backpressure check (`_enrichment_queue_depth()`, direct Redis `LLEN` on the `enrichment` queue) that skips dispatch entirely above `ENRICHMENT_QUEUE_BACKPRESSURE_THRESHOLD` (default 5000) |
| E13 | Partially-persisted SBOM/VEX on transient ingest failure | D      | `ingest` queue, graph integrity | Low        | Medium | Low    | `ingest_cyclonedx`/`ingest_spdx`/`ingest_vex` (in `ingest_tasks.py`, consumed from the dedicated `ingest` queue -- see `docs/ingest-pipeline.md`) issue several separate graph writes rather than one atomic transaction, so a transient failure partway through can leave a partial write. Mitigated by: automatic retry (3x, 30s backoff) on transient exceptions, safe because every write is MERGE-based on deterministic keys so a retry converges to a complete graph rather than duplicating. `_retry_or_sanitised_error()` checks `task.request.called_directly` and falls back to the pre-existing sanitised error result when a task is invoked directly rather than dispatched via the broker (e.g. `ingest_sbom`'s nested calls to `ingest_cyclonedx`/`ingest_spdx`), since `self.retry()` cannot reschedule outside a real worker context and would otherwise re-raise the raw exception, leaking detail (CWE-209). |
...
5. **E9** (Low, reduced from Medium): Queue-depth monitoring is now implemented as an automated backpressure check rather than a manual monitoring task; remaining gap is that Celery `task_time_limit` is still not set on `enrich_package`, so a genuinely hung certifier HTTP call has no hard timeout beyond the certifier's own `httpx` timeout
6. **E13** (Low): Consider alerting on repeated `ingest` task retries for the same `record_id`, which would indicate a persistent (not transient) failure that retries alone won't resolve
...
| Date       | Change                                                                 |
| ---------- | ---------------------------------------------------------------------- |
| 2026-07-28 | Doc/implementation audit. Updated E9 to reflect the `enrichment_queued_at` dispatch-time guard and queue-depth backpressure check added to `enrich_all_packages` after an actual incident (unbounded fan-out filled Redis to `maxmemory`, OOM-looped the worker). Added E13 for the new retry-with-backoff behaviour in `ingest_tasks.py` and its CWE-209-safe fallback for direct (non-broker) invocation. Verified certifier rate limits (OSV 100, ClearlyDefined n/a, Scorecard 30, OSS Index 60/120, deps.dev 150, source_repo 100, EOL 30 req/min) and trust-score weights/alpha/decay/max_depth against current code -- no drift found there. Note: this threat model's Architecture diagram and Trust Boundaries table still don't mention the `ingest` queue / dedicated ingest worker pool / `ingest_tasks.py` at all -- that's a larger pre-existing gap (the async-ingest pipeline was added to this codebase before this session and this document was never updated for it), flagged but not fully addressed in this pass. |
