...
4. Use `SonaTypeIngestor` from `data-ingestion` to fetch and ingest SBOM data
...
```text
release-listener/
├── src/release_listener/     # Main application code
│   ├── __init__.py
...
├── helm/release-listener/   # Helm chart for Kubernetes
│   ├── Chart.yaml
│   ├── values.yaml
...
| Dependency | Purpose |
| ------------ | --------- |
| `data-ingestion` | SonaType API and FalkorDB persistence |
| `flask` | Web framework for webhook handling |
| `gunicorn` | Production WSGI server |
| `falkordb` | Graph database client (used by data-ingestion) |

### data-ingestion Library

The `data-ingestion` library is a local dependency. Key classes:
...
SONATYPE_USERNAME=user SONATYPE_PASSWORD=pass uv run python -m release_listener.app
...
docker run -p 8000:8000 \
  -e SONATYPE_USERNAME=user -e SONATYPE_PASSWORD=pass release-listener:latest
...
The Helm chart (`helm/release-listener/`) provides:
...
Key Helm values (see `helm/release-listener/values.yaml` for full list):
...
```python
@patch('release_listener.app.SonaTypeIngestor')
...
```python
@patch('data_ingestion.sonatype_ingestor.SonaTypeIngestor.get_existing_data')
@patch('data_ingestion.sonatype_ingestor.Lifecycle')
...
1. Add route in `src/release_listener/app.py` inside `create_app()`
...
snyk code test /path/to/release-listener
```
