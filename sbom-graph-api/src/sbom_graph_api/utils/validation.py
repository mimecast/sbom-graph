...
PURL_PATTERN = re.compile(
    r"^pkg:[a-zA-Z][a-zA-Z0-9._+-]*/"  # scheme + type
...
def validate_purl(value: str) -> str | None:
    """Validate a package URL (purl) for safe use.

    Accepts URL-decoded purl strings matching the purl-spec format.

    Args:
        value: The purl string to validate

    Returns:
        The validated purl, or None if invalid
    """
    if not value:
        return None
    ...
    if not PURL_PATTERN.match(value):
        return None

    return value


def validate_project_group(value: str | None) -> str | None:
    """Validate an optional project group query parameter.

    Uses the same safe character set as project names.

    Args:
        value: The project group to validate

    Returns:
        The validated project group, or None if absent or invalid
    """
    if not value:
        return None
