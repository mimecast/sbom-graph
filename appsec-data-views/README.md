...
docker run -p 8080:8080 \
  ...
  --timeout 300 --max-requests 1000 --max-requests-jitter 50 appsec_data_views.wsgi:app
...
uv run gunicorn -c gunicorn.conf.py appsec_data_views.wsgi:app
...
A Helm chart is provided in `charts/appsec-data-views/`.

### Install

```bash
helm install appsec-data-views ./charts/appsec-data-views \
...
```bash
uv run pytest --cov=src/appsec_data_views --cov-report=term-missing
```
